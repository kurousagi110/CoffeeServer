...
router.post("/tim-kiem-san-pham-theo-category", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.timKiemSanPhamTheoCategory(
      ten_loai_san_pham
    );
    if (san_pham) {
      res.status(200).json({
...
router.get("/tim-kiem-san-pham/:ten_san_pham", async (req, res) => {
  try {
    const san_pham = await sanPhamController.timKiemSanPham(
      req.params.ten_san_pham
    );
    if (san_pham) {
      res.status(200).json({
        success: true,
...
router.get("/get-san-pham-by-id/:id_san_pham", async (req, res) => {
  try {
    const san_pham = await sanPhamController.getSanPhamById(
      req.params.id_san_pham
    );
    if (san_pham) {
      res.status(200).json({
        success: true,
...
router.post("/them-loai-san-pham", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.themLoaiSanPham(
      id_san_pham,
      ten_loai_san_pham,
      ma_loai_san_pham
    );
...
router.post("/sua-loai-san-pham", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.suaLoaiSanPham(
      id_san_pham,
      id_loai_san_pham,
      ten_loai_san_pham,
      ma_loai_san_pham
...
router.post("/xoa-loai-san-pham", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.xoaLoaiSanPham(
      id_san_pham,
      id_loai_san_pham
    );
    if (san_pham) {
...
router.post("/them-anh", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.themHinhAnh(
      id_san_pham,
      hinh_anh_sp
    );
    if (san_pham) {
...
router.post("/xoa-anh", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.xoaHinhAnh(
      id_san_pham,
      id_hinh_anh_sp
    );
    if (san_pham) {
...
router.post("/upload-anh", [uploadAnh.single("image")], async (req, res) => {
  try {
    ...
    if (file) {
      ...
      res
        .status(200)
        .json({ result: true, message: "Upload ảnh thành công", data: path });
    } else {
...
router.post("/them-size", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.themSize(
      id_san_pham,
      ten_size,
      gia,
      giam_gia
...
router.post("/sua-size", async (req, res) => {
  try {
    ...
    const san_pham = await sanPhamController.suaSize(
      id_san_pham,
      id_size,
      ten_size,
      gia,
