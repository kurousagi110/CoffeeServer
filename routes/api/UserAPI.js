...
router.post("/login-cpanel", async (req, res, next) => {
  try {
    ...
    if (result) {
      res.status(200).json({
        trang_thai: true,
        data: { id_user: result.user._id, token: result.token },
      });
    } else {
      res
        .status(200)
...
router.post("/sua-dia-chi", AuthenToken, async (req, res, next) => {
  try {
    ...
    const result = await userController.suaDiaChi(
      id_user,
      id_dia_chi,
      ten_dia_chi,
      so_dien_thoai,
...
router.post("/them-dia-chi", AuthenToken, async (req, res, next) => {
  try {
    ...
    const result = await userController.themDiaChi(
      id_user,
      ten_dia_chi,
      so_dien_thoai,
      so_nha,
...
router.post("/sua-user", AuthenToken, async (req, res, next) => {
  try {
    const { id_user, ho_ten, avatar, email, so_dien_thoai, device_token } =
      req.body;
    const result = await userController.suaThongTinUser(
      id_user,
      ho_ten,
      avatar,
      email,
...
router.post("/doi-mat-khau", AuthenToken, async (req, res, next) => {
  try {
    ...
    const result = await userController.doiMatKhau(
      id_user,
      mat_khau_cu,
      mat_khau_moi
    );
    if (result) {
      res
        .status(200)
        .json({ trang_thai: true, message: "Đổi mật khẩu thành công" });
    } else {
      res
        .status(200)
...
router.post("/xoa-tai-khoan", AuthenToken, async (req, res, next) => {
  try {
    ...
    if (result) {
      res
        .status(200)
        .json({ trang_thai: true, message: "Xóa user thành công" });
    } else {
...
router.post("/dang-nhap-sdt", async (req, res, next) => {
  try {
    ...
    const result = await userController.dangNhapBangSoDienThoai(
      so_dien_thoai,
      mat_khau
    );
    if (result) {
...
router.post("/dang-ky-sdt", async (req, res, next) => {
  try {
    ...
    const result = await userController.dangKyBangSoDienThoai(
      so_dien_thoai,
      mat_khau,
      ho_ten
    );
    if (result) {
      ...
      res
        .status(200)
        .json({ trang_thai: false, message: "Số điện thoại bị trùng" });
    }
  } catch (error) {
    res.status(400).json({ trang_thai: false, message: error.message });
  }
...
router.post("/dang-nhap-email", async (req, res, next) => {
  try {
    ...
    if (result) {
      ...
      res
        .status(200)
        .json({ trang_thai: false, message: "Đăng nhập bằng email thất bại" });
    }
  } catch (error) {
    res.status(400).json({ trang_thai: false, message: error.message });
  }
...
router.post("/dang-nhap-username", async (req, res, next) => {
  try {
    const { tai_khoan, mat_khau } = req.body;
    const result = await userController.dangNhapBangUsername(
      tai_khoan,
      mat_khau
    );
    if (result) {
...
router.post("/gui-otp", async (req, res, next) => {
  try {
    ...
    if (result) {
      res
        .status(200)
        .json({ status: 1, notification: "Gửi mã otp thành công" });
    } else {
      res
        .status(200)
        .json({ status: 0, notification: "Gửi mã otp không thành công" });
    }
  } catch (error) {
    console.log("Gửi otp lỗi: ", error);
    res
      .status(400)
      .json({ status: 0, message: "Gửi mã otp lỗi" + error.message });
  }
});
//kiểm tra otp
...
router.post("/kiem-tra-otp", async (req, res, next) => {
  try {
    ...
    if (result) {
      res
        .status(200)
        .json({ status: 1, notification: "Kiểm tra mã otp thành công" });
    } else {
      res
        .status(200)
        .json({ status: 0, notification: "Kiểm tra mã otp không thành công" });
    }
  } catch (error) {
    console.log("Kiểm tra otp lỗi: ", error);
    res
      .status(400)
      .json({ status: 0, message: "Kiểm tra mã otp lỗi" + error.message });
  }
});
